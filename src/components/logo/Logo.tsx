...
const Logo = forwardRef(({ disabledLink = false, sx, ...other }: LogoProps, ref) => {
  ...
  const logo = (
    <Box
      ...
      {...other}
    >
      <Box component="img" alt="logo" src={images.logo.logo_mbkc} sx={{ width: 300, height: 150 }} />
      <Box component="img" alt="logo text" src={images.logo.logo_mbkc_text} sx={{ height: 80, mt: 2.5 }} />
    </Box>
  );
